...
export default function NewInvoicePage() {
  ...
  const [form, setForm] = useState({ client_id: '', job_id: '' })
  ...
  const save = async (send: boolean) => {
    if (!lineItems.some(i => i.description && i.rate > 0)) return
    setSaving(true)
    try {
    ...
    <div className="p-4 lg:p-6 max-w-3xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:gap-4">
          <div>
            ...
            <select value={form.client_id} onChange={e => setForm({...form, client_id: e.target.value})}
              className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white outline-none focus:border-gray-400">
        ...
        <div>
          ...
          <div className="border border-gray-200 rounded-xl overflow-hidden">
            <div className="grid grid-cols-12 gap-0 bg-gray-50 px-3 py-2 text-xs text-gray-500 font-medium border-b border-gray-200">
              ...
              <span className="col-span-3 text-right">Rate (ex GST)</span>
            ...
            {lineItems.map((item, i) => (
              <div key={i} className="grid grid-cols-12 gap-0 px-3 py-2 border-b border-gray-100 last:border-0 items-center">
                <input value={item.description} onChange={e => updateLine(i, 'description', e.target.value)}
                  className="col-span-6 text-sm outline-none pr-2" placeholder="Description..." />
                <input type="number" value={item.qty} onChange={e => updateLine(i, 'qty', parseFloat(e.target.value) || 1)}
                  className="col-span-2 text-sm outline-none text-center" min={1} />
                <input type="number" value={item.rate || ''} onChange={e => updateLine(i, 'rate', parseFloat(e.target.value) || 0)}
                  className="col-span-3 text-sm outline-none text-right" placeholder="0.00" />
        ...
        <div className="text-right space-y-1 text-sm">
          <div className="text-gray-500">Subtotal: <span className="text-gray-900 font-medium">${subtotal.toFixed(2)}</span></div>
