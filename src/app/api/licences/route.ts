...
export async function GET() {
  try {
    ...
    await query("ALTER TABLE licences ADD COLUMN IF NOT EXISTS image_type VARCHAR(100)").catch(() => {})
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await query(
      'INSERT INTO licences (name, type, licence_number, issue_date, expiry_date, no_expiry, image_url, image_type, notes) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *',
      [data.name, data.type || 'licence', data.licence_number || null, data.issue_date || null,
       data.no_expiry ? null : (data.expiry_date || null), data.no_expiry || false, data.image_url || null, data.image_type || null, data.notes || null]
    )
...
export async function PUT(req: NextRequest) {
  try {
    const data = await req.json()
    const result = await query(
      'UPDATE licences SET name=$1, type=$2, licence_number=$3, issue_date=$4, expiry_date=$5, no_expiry=$6, image_url=$7, image_type=$8, notes=$9 WHERE id=$10 RETURNING *',
      [data.name, data.type, data.licence_number, data.issue_date || null,
       data.no_expiry ? null : (data.expiry_date || null), data.no_expiry || false, data.image_url || null, data.image_type || null, data.notes, data.id]
    )
    return NextResponse.json(result.rows[0])
  } catch (error: any) {
    return NextResponse.json({ error: error.message }, { status: 500 })
